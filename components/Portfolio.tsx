...
import { YOUTUBE_IDS, PROJECTS } from '../constants';
    ...
    <div className="max-w-7xl mx-auto px-4">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
        <div>
          ...
          <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tighter">Latest Productions</h3>
        </div>
        <p className="text-zinc-500 max-w-md text-right md:text-right hidden md:block">
          Explore our portfolio of high-energy music videos and creative visuals produced right in the heart of LA.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {PROJECTS.map((project, index) => (
          <div 
            ...
            onClick={() => setSelectedVideo(YOUTUBE_IDS[index % YOUTUBE_IDS.length])}
          >
            <div className="relative aspect-video overflow-hidden rounded-lg bg-zinc-900 border border-zinc-800">
              <img 
                src={`https://img.youtube.com/vi/${YOUTUBE_IDS[index % YOUTUBE_IDS.length]}/maxresdefault.jpg`}
                alt={project.title}
            ...
            <h4 className="mt-4 text-xl font-bold group-hover:text-yellow-400 transition-colors italic uppercase tracking-tighter">
              {project.title}
            </h4>
          </div>
        ))}
      ...
      {selectedVideo && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95">
          <button 
            className="absolute top-10 right-10 text-white hover:text-yellow-400 text-4xl"
            onClick={() => setSelectedVideo(null)}
          >
            <i className="fa-solid fa-times"></i>
          </button>
          <div className="w-full max-w-5xl aspect-video bg-zinc-900 shadow-2xl shadow-yellow-400/20">
            <iframe 
